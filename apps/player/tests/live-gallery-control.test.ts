...
describe("live Gallery control", () => {
  ...
  it("applies a valid changed child index-first and cleans up", () => {
    ...
    const controller = { getCurrentIndex: vi.fn(() => 0), getCurrentSlide: vi.fn(() => presentation().slides[0]), setGalleryActiveIndex: vi.fn(), setGalleryExpanded: vi.fn() };
