...
describe("public Watch runtime route", () => {
  ...
  it("serves the exact cache-clearing technical response only on its dedicated route", () => {
    ...
    expect(config.rewrites).toContainEqual({
      source: "/__player/clear-cache",
      destination: "/__player/clear-cache.html",
    });
    expect(config.headers?.find((rule) => rule.source === "/__player/clear-cache")?.headers).toEqual([
  ...
  it("keeps the technical return page same-origin and safely bounded", async () => {
    const source = await import.meta.glob("../public/__player/clear-cache.html", { query: "?raw", import: "default", eager: true });
