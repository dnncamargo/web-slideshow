  ...
  return <>
    ...
    <InspectorSection title={t("inspector.content")} count={element.items.length} defaultOpen>
      ...
      <div className={styles.galleryItemSelector} role="group" aria-label={t("gallery.items", { count: element.items.length })}>
        {element.items.map((_item, index) => <button key={index} type="button" className={`${styles.secondaryButton} ${styles.galleryItemSelectorButton} ${index === selectedItemIndex ? styles.galleryItemSelectorButtonSelected : ""}`} aria-label={t("gallery.imageNumber", { number: index + 1 })} aria-pressed={index === selectedItemIndex} data-powershow-gallery-select="true" data-powershow-gallery-index={index} onClick={() => onSelectedItemIndexChange(index)}>{t("element.image")} {index + 1}</button>)}
      </div>
      <div className={styles.galleryItemActions}>
        ...
        {selectedItem && selectedItemIndex !== null && selectedItemIndex !== undefined && <>
          <button type="button" className={`${styles.secondaryButton} ${styles.galleryItemActionIcon}`} disabled={selectedItemIndex === 0} aria-label={t("gallery.moveUp")} data-powershow-gallery-move-up="true" onClick={() => moveItem(-1)}>↑</button>
          <button type="button" className={`${styles.secondaryButton} ${styles.galleryItemActionIcon}`} disabled={selectedItemIndex === element.items.length - 1} aria-label={t("gallery.moveDown")} data-powershow-gallery-move-down="true" onClick={() => moveItem(1)}>↓</button>
