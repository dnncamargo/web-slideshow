      ...
      <div className={styles.collectionSelector} role="group" aria-label={t("gallery.items", { count: element.items.length })}>
        {element.items.map((item, index) => {
          ...
          return <div key={index} className={styles.collectionSelectorRow}>
            ...
            <button type="button" className={`${styles.secondaryButton} ${styles.collectionSelectorButton} ${index === selectedItemIndex ? styles.collectionSelectorButtonSelected : ""}`} aria-label={`${index + 1}. ${name}`} title={name} aria-pressed={index === selectedItemIndex} data-presentation-gallery-select="true" data-presentation-gallery-index={index} onClick={() => onSelectedItemIndexChange(index)}><span className={styles.collectionItemName}>{name}</span></button>
          </div>;
        })}
      </div>
      <div className={styles.galleryItemActions}>
        <button type="button" className="ps-ui-action" data-presentation-gallery-add="true" onClick={addItem}>{t("gallery.add")}</button>
        {selectedItem && selectedItemIndex !== null && selectedItemIndex !== undefined && <>
          <button type="button" className="ps-ui-action" aria-label={t("inspector.remove")} data-presentation-gallery-remove="true" onClick={removeItem}><span>{t("inspector.remove")}</span></button>
        </>}
      </div>
      {selectedItem && selectedItemIndex !== null && selectedItemIndex !== undefined ? <>
