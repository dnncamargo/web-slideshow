  ...
  return (
    <>
      ...
      <InspectorSection title={t("inspector.content")} defaultOpen>
        ...
        <div className={styles.galleryItemActions}><button type="button" className="ps-ui-action" data-powershow-scripted-port-add="true" onClick={addPort}>+ {t("scripted.addPort")}</button>{selectedPort && <button type="button" className="ps-ui-action" data-powershow-scripted-port-remove="true" aria-label={t("inspector.remove")} onClick={removeSelectedPort}><span>{t("inspector.remove")}</span></button>}</div>
        {selectedPort && <>
