...
export interface ControlScriptedStatePort { portIndex: number; portId: string; label: string; kind: StatefulKind; direction: Direction; min?: number; max?: number; step?: number; runtimeAvailable: boolean; desiredValue: boolean | number | null; desiredRevision: number | null; reportedValue: boolean | number | null; reportRevision: number | null; appliedInputRevision: number | null; status: ControlScriptedPortStatus; writable: boolean; }
...
function viewPort(current: Latest, group: DescriptorGroup, port: DescriptorPort): ControlScriptedStatePort {
  const runtime = runtimeFor(current, group); if (!runtime) return { ...port, runtimeAvailable: false, desiredValue: null, desiredRevision: null, reportedValue: null, reportRevision: null, appliedInputRevision: null, status: "unavailable", writable: false };
  ...
  return { ...port, runtimeAvailable: true, desiredValue: input?.value ?? null, desiredRevision: input?.revision ?? null, reportedValue: report?.value ?? null, reportRevision: report?.revision ?? null, appliedInputRevision: report?.appliedInputRevision ?? null, status, writable: safe(current) && (port.direction === "input" || port.direction === "input-output") };
}
...
export function useLiveScriptedStateControl(options: UseLiveScriptedStateControlOptions): UseLiveScriptedStateControlResult {
  ...
  const [hydrated, setHydrated] = useState<Hydrated>({ runtime: null, input: null, report: null }); const [sendFailed, setSendFailed] = useState(false);
  ...
  useEffect(() => { setSendFailed(false); }, [options.live?.revision, options.live?.currentVersionId, options.desiredPageId]);
  const latest = useRef<Latest>({ ...options, groups: descriptors, hydrated }); latest.current = { ...options, groups: descriptors, hydrated };
  const setPortValue = useCallback((slot: number, portIndex: number, value: boolean | number) => { const current = latest.current; const found = find(current, slot, portIndex); const runtime = found && runtimeFor(current, found.group); if (!safe(current) || !found || !runtime || (found.descriptor.direction !== "input" && found.descriptor.direction !== "input-output") || !valid(found.descriptor, value) || current.live === null || current.desiredPageId === null || current.playerStatus?.kind !== "ready") return; const database = getRealtimeDatabaseOrNull(); setSendFailed(false); if (!database) { setSendFailed(true); return; } const command = { activationRevision: current.live.revision, currentVersionId: current.live.currentVersionId, pageId: current.desiredPageId, scriptedSlot: found.group.scriptedSlot, elementId: found.group.elementId, portIndex: found.descriptor.portIndex, portId: found.descriptor.portId, targetBootId: runtime.bootId, targetMountRevision: runtime.mountRevision, value }; void writeScriptedInput(database, command).catch(() => { const after = latest.current; const next = find(after, command.scriptedSlot, command.portIndex); const nextRuntime = next && runtimeFor(after, next.group); if (next && next.group.elementId === command.elementId && next.descriptor.portId === command.portId && after.live?.revision === command.activationRevision && after.live?.currentVersionId === command.currentVersionId && after.desiredPageId === command.pageId && nextRuntime?.bootId === command.targetBootId && nextRuntime.mountRevision === command.targetMountRevision) setSendFailed(true); }); }, []);
  const current = latest.current; return { groups: descriptors.map((group) => ({ ...group, ports: group.ports.map((port) => viewPort(current, group, port)) })), controlsEnabled: safe(current), sendFailed, setPortValue };
}
