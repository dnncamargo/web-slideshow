...
function presentation(): Presentation {
  return PresentationSchema.parse({
    ...
    rootDefinitions: [{
      ...
      root: container("root-container", [text("root-text", "Master content")]),
    }],
  });
}
...
describe("SM6C Root Definition workspace shell", () => {
  ...
  function render(initialPresentation = presentation(), onSave = vi.fn()): void {
    act(() => {
      root.render(
        <StudioI18nProvider>
          <EditorWorkspace
  ...
  it("renders the existing Root Definition projection without adding a Slide", () => {
    ...
    expect(containerElement.textContent).toContain("Slide mestre · Teaching master");
  ...
  it("explicitly exits to the retained Slide without saving or changing Presentation", () => {
    ...
    expect(containerElement.textContent).not.toContain("Slide mestre · Teaching master");
    expect(source.slides).toHaveLength(2);
