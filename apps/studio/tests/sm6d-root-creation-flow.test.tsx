...
describe("SM6D Slide and Root Definition creation flow", () => {
  ...
  async function chooseRoot(name: string, preset: string): Promise<void> {
    const rootRadio = container.querySelector<HTMLInputElement>('input[value="root-definition"]');
    if (!rootRadio) throw new Error("expected Root Definition creation choice");
    await act(async () => rootRadio.click());
    expect(exactButtonByText(container, "Create Root Definition").disabled).toBe(true);
    await act(async () => buttonByText(container, preset).click());
    const nameInput = container.querySelector<HTMLInputElement>('input[aria-label="Root Definition name"]');
    if (!nameInput) throw new Error("expected Root Definition name field");
    await act(async () => changeInput(nameInput, name));
    await act(async () => exactButtonByText(container, "Create Root Definition").click());
  }
  ...
  it("creates a trimmed two-column Root in the same workspace with one history action", async () => {
    const saved: Presentation[] = [];
    await mount(presentation(), saved);
    await openNew();
    await chooseRoot("  Teaching master  ", "Two Columns");
    ...
    expect(created?.name).toBe("Teaching master");
    expect(created?.root.type).toBe("container");
    expect(created?.root.children).toHaveLength(2);
    expect(container.querySelector('[data-authoring-target="root-definition"]')).not.toBeNull();
    expect(container.textContent).toContain("Master slide · Teaching master");
    expect(container.querySelector("[class*='layoutPicker']")).toBeNull();
  ...
  it("creates a blank Root, then undo removes it and redo restores the document", async () => {
    ...
    await mount(presentation(), saved);
    await openNew();
    await chooseRoot("Blank master", "Blank");
    await save();

    const created = saved.at(-1)?.rootDefinitions?.[0];
    expect(created?.root).toMatchObject({ type: "container", hidden: false, children: [] });
  ...
  it("supports multiple Roots in Root mode and can create a Slide from Root mode", async () => {
    ...
    await mount(presentation(), saved);
    await openNew();
    await chooseRoot("Root A", "Blank");
    await openNew();
    await chooseRoot("Root B", "Title + 2 Cols");
    await save();

    const afterRoots = saved.at(-1);
    expect(afterRoots?.rootDefinitions).toHaveLength(2);
    expect(afterRoots?.rootDefinitions?.map((definition) => definition.name)).toEqual(["Root A", "Root B"]);
    ...
    await openNew();
    const slideRadio = container.querySelector<HTMLInputElement>('input[value="slide"]');
    if (!slideRadio) throw new Error("expected Slide creation choice");
    await act(async () => slideRadio.click());
    await act(async () => exactButtonByText(container, "+ New").click());
    await save();
