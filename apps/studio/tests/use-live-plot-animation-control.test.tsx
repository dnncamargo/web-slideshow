...
describe("useLivePlotAnimationControl", () => {
  ...
  beforeEach(async () => {
    ...
    input = { live: LIVE, effectiveSlide: effective([plot("plot-a")]), desiredPageId: "page-a", actualPageId: "page-a", controlSynced: true, controlsBlocked: false, playerStatus: READY };
  ...
  it("discovers animated Plots in deep canonical order and normalizes labels", () => {
    const targets = discoverLivePlotAnimationTargets(effective([
      plot("a", " y =   sin(x) "),
  ...
  it("prevents a duplicate pending target and sends all targets independently", async () => {
    ...
    input = { ...input, effectiveSlide: effective([plot("plot-a"), plot("plot-b")]) }; await render();
  ...
  it("ignores a late failure after page and boot context changes", async () => {
    ...
    await act(async () => result?.triggerAction(result.plotTargets[0]!, "play"));
    input = { ...input, desiredPageId: "page-b", actualPageId: "page-b", effectiveSlide: effective([], [plot("plot-b")], "page-b"), playerStatus: { ...READY, presence: { ...READY.presence, bootId: "boot-b" } } }; await render();
