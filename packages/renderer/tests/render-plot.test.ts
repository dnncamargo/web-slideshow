...
describe("Plot renderer", () => {
  ...
  it("applies transient bindings to explicit-z geometry", () => {
    ...
    expect(initial).toContain("presentation-plot-surface-svg");
    expect(transient).toContain("presentation-plot-surface-svg");
    expect(transient).not.toBe(initial);
  });
  ...
  it("fits a positive constant explicit-y without manufacturing an x-axis", () => {
    ...
    expect(html).not.toContain("presentation-plot-axis-x");
    expect(html).toContain("presentation-plot-axis-y");
    expect(html).not.toContain('viewBox="0 0 20 20"');
  });

  it("fits a positive constant explicit-x with only the x-axis visible", () => {
    const html = renderPlot("x = 5", { fitToAxes: false });
    expect(html).toContain("presentation-plot-axis-x");
    expect(html).not.toContain("presentation-plot-axis-y");
  });
  ...
  it.each([
    ...
    expect(html).not.toContain("presentation-plot-svg");
  });
  ...
  it.each([undefined, true] as const)("shows explicit-z axes by default or when showAxes=%s", (showAxes) => {
    ...
    expect(html).toContain("presentation-plot-axis-x");
    expect(html).toContain("presentation-plot-axis-y");
    expect(html).toContain("presentation-plot-axis-z");
  });
  ...
  it("hides explicit-z axes without hiding the surface", () => {
    ...
    expect(html).toContain("presentation-plot-surface-wireframe");
    expect(html).not.toContain("presentation-plot-axis");
  });
  ...
  it("hides 2D axes without changing the rendered curve", () => {
    ...
    expect(shown).toContain("presentation-plot-axis");
  ...
  it("passes explicit-z Plot Z colors into the mathematical gradient renderer", () => {
    ...
    expect(html).toContain("presentation-plot-surface-wireframe-z-gradient");
  ...
  it("keeps zGradient inert for 2D Plot output", () => {
    ...
    expect(html).toContain("presentation-plot-svg");
    expect(html).not.toContain("presentation-plot-surface-wireframe-z-gradient");
    expect(html).toContain("color:#ff0000");
  });
  ...
  it.each(["z = x + y", "z = 2"])("renders explicit-z surface %s", (source) => {
    expect(renderPlot(source)).toContain("presentation-plot-surface-svg");
  });

  it("retains a surface around local domain gaps", () => {
    expect(renderPlot("z = sqrt(x)")).toContain("presentation-plot-surface-svg");
  });
  ...
  it("falls back when multiple explicit-z equations are present", () => {
    const html = renderPlot("z = x + y\nz = x - y");
    expect(html).toContain("presentation-placeholder-plot");
    expect(html).not.toContain("presentation-plot-surface-svg");
  });
  ...
  it.each([true, false])("keeps explicit-z output valid regardless of fitToAxes=%s", (fitToAxes) => {
    expect(renderPlot("z = x + y", { fitToAxes })).toContain("presentation-plot-surface-svg");
  });
  ...
  it("preserves the existing Interactive placeholder path", () => {
    ...
    expect(renderElement(element)).toBe(
      '<div class="presentation-element presentation-placeholder presentation-placeholder-interactive" data-presentation-id="interactive-test" data-presentation-type="interactive">[interactive]</div>',
    );
  });
});
